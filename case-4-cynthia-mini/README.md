...
В этой работе я сделала небольшую учебную версию ATS-системы Cynthia. Интерфейс открывается в браузере, серверная часть работает на Delphi 10.2 WebBroker через IIS, а данные хранятся в Microsoft SQL Server.
...
Я сознательно не добавляла аутентификацию, загрузку резюме, отправку сообщений, AI-рейтинг и интеграции исходной Cynthia. Интерфейс работает после публикации DLL в IIS — это не отдельный статический макет.

## Демонстрация работы

Для скриншотов я запустила интерфейс с тестовыми ответами API и оставила масштаб браузера 100%. На изображениях показаны основные сценарии работы.
...
Я использовала одинаковую разрядность DLL, пула IIS и драйвера SQL Server. Если разрядность различается, приложение не загрузится.

## 1. Как воспроизвести базу данных

Я создавала базу через SQL Server Management Studio. Повторить это можно так:
...
3. Выполнить `database/smoke-test.sql`. Для проверки я подготовила 8 вакансий, 15 кандидатов и 18 откликов на разных этапах.

Я сделала скрипт повторяемым: он не удаляет существующую базу и не добавляет тестовые записи второй раз, если в `Companies` уже есть данные.

## 2. Как воспроизвести сборку Delphi

Проект я собирала в Delphi 10.2. Последовательность действий:
...
10. Открыть корень сайта. Я дополнительно проверяла ответ `/api/dashboard` — это помогает понять, работает ли серверная часть.
...
`config.ini` я не добавляла в Git, потому что в нём могут находиться логин и пароль от базы. Файл уже указан в `.gitignore`.
...
В текущем виде я считаю проект учебным прототипом. Для реальной эксплуатации я бы добавила JWT- или Windows-аутентификацию, роли, CSRF/CORS-политику, полный журнал аудита, транзакции, внешнее хранение файлов, пагинацию, тесты API, защищённое хранение секретов и HTTPS.
