...
Для решения задачи я написал программу, которая:
...
- Для поиска значений и их позиций я использовал `min()`, `max()` и `index()`. При повторении минимума или максимума `index()` берёт первое вхождение, и это влияет на выбранный участок.
...
Я запускал программу на Python 3. Чтобы воспроизвести результат, из корня репозитория можно выполнить:

```bash
python case-1-array/main.py
```

Для проверки я ввёл `N = 8` и массив `[4, -3, 12, -5, -7, 2, -1, -10]`. Между максимальным элементом `12` и минимальным элементом `-10` находятся числа `-5`, `-7` и `-1`, поэтому программа вывела `-13`.
