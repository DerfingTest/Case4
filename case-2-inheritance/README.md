...
В `Chief.report()` я использовала `super().report()`: сначала выполняется метод базового класса, затем руководитель сообщает о работе всего отдела. В тестовой части я создаю по одному объекту каждого класса и показываю обычный, унаследованный, переопределённый и собственный методы.
...
- Я переопределила `report()` в `Chief`, и один вызов ведёт себя по-разному для сотрудника и руководителя. Так в программе проявляется полиморфизм.
...
- В итоге общую логику сотрудников я оставила в `Employee`, а особенности руководителя вынесла в `Chief`. Это уменьшает повторение кода и упрощает дальнейшее расширение программы.

## Как воспроизвести результат

Я запускала программу на Python 3. Чтобы увидеть тот же результат, из корня репозитория можно выполнить:
